...
public class Listener extends Controller {
    ...
    private boolean[] getNewRelayStatus() {
        ...
        try {
            ...
            TypedQuery<Double> query = JPA.em().createQuery("SELECT sr.temperature FROM SensorReadings AS sr ORDER BY sr.timestamp DESC", Double.class);
            latestReading = query.setMaxResults(1).getResultList().get(0);
        } catch (Exception e) {
